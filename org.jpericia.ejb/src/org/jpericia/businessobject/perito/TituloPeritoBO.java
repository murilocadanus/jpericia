...
import org.jpericia.dao.perito.TituloPeritoDAO;
...
public class TituloPeritoBO extends AbstractBusinessObject {
	
	private static Logger logger = Logger.getLogger(TituloPeritoBO.class);
	
	private static TituloPeritoBO me;

	private TituloPeritoBO() {
	}

	private static synchronized void loadInstance() {
		if (me == null) {
			me = new TituloPeritoBO();
		}
	}
	 ...
	 * @return ClienteAS
	 */
	public static TituloPeritoBO getInstance() {
		if (me == null) {
			loadInstance();
		}
		return me;
	 ...
	 */
	public void inserir(TituloPerito tituloPerito) throws BusinessObjectException{
		try{
			logger.debug("Inicio " + this.getClass().getName() + ".salvar()");
			JPericiaDAOFactory factory = this.getDaoFactory();
			TituloPeritoDAO tituloPeritoDAO = (TituloPeritoDAO)factory.getDao(TituloPeritoDAO.class);
			tituloPeritoDAO.inserir(tituloPerito);
	 ...
	 * @throws ApplicationServiceException
	 */
	public void atualizar(TituloPerito tituloPerito) throws BusinessObjectException{
		try{
			logger.debug("Inicio " + this.getClass().getName() + ".salvar()");
			JPericiaDAOFactory factory = this.getDaoFactory();
			TituloPeritoDAO tituloPeritoDAO = (TituloPeritoDAO)factory.getDao(TituloPeritoDAO.class);
			tituloPeritoDAO.atualizar(tituloPerito);
			logger.debug("Fim " + this.getClass().getName() + ".salvar()");
		}catch (JPericiaDAOFactoryException tdfe) {
			logger.fatal(tdfe.getMessage(), tdfe);
			throw new BusinessObjectException("Erro salvando Cliente", tdfe);
	 ...
	 * @throws ApplicationServiceException
	 */
	public void remover(TituloPerito tituloPerito) throws BusinessObjectException{
		try{
			logger.debug("Inicio " + this.getClass().getName() + ".salvar()");
			JPericiaDAOFactory factory = this.getDaoFactory();
			TituloPeritoDAO tituloPeritoDAO = (TituloPeritoDAO)factory.getDao(TituloPeritoDAO.class);
			tituloPeritoDAO.remover(tituloPerito);
			logger.debug("Fim " + this.getClass().getName() + ".salvar()");
		}catch (JPericiaDAOFactoryException tdfe) {
			logger.fatal(tdfe.getMessage(), tdfe);
			throw new BusinessObjectException("Erro salvando Cliente", tdfe);
	...
	public ArrayList<AbstractEntity> pesquisar() throws BusinessObjectException{
		try{
			...
			JPericiaDAOFactory factory = this.getDaoFactory();
			TituloPeritoDAO tituloPeritoDAO = (TituloPeritoDAO)factory.getDao(TituloPeritoDAO.class);
			ArrayList<AbstractEntity> pesquisa = tituloPeritoDAO.pesquisar();
			logger.debug("Fim " + this.getClass().getName() + ".salvar()");
			return pesquisa;
		}catch (JPericiaDAOFactoryException tdfe) {
			logger.fatal(tdfe.getMessage(), tdfe);
	...
	public PaginaTO consultar(CriterioPesquisaTO criterios) throws BusinessObjectException{
		...
		try{
			...
			JPericiaDAOFactory factory = this.getDaoFactory();
			TituloPeritoDAO tituloPeritoDAO = (TituloPeritoDAO)factory.getDao(TituloPeritoDAO.class);
			retorno = tituloPeritoDAO.consultar(criterios);
