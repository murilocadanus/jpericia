...
public class TituloPeritoDAOJPA extends AbstractDAO implements TituloPeritoDAO {
	
	private static Logger logger = Logger.getLogger(TituloPeritoDAOJPA.class); 
	
	//@Override
	public void inserir(TituloPerito tituloPerito) throws DAOException {
		logger.debug("Entrou ClienteDAOJPA");
		EntityManager manager = null;
		try{
			manager = PersistenceUtil.currentEntityManager();
			manager.persist(tituloPerito);
			manager.flush();
		}catch(Exception e){
	...
	public void atualizar(TituloPerito tituloPerito) throws DAOException {
		logger.debug("Entrou ClienteDAOJPA");
		EntityManager manager = null;
		try{
			manager = PersistenceUtil.currentEntityManager();
			TituloPerito tituloPeritoManaged = manager.merge(tituloPerito);
			
			manager.persist(tituloPeritoManaged);
			manager.flush();
		}catch(Exception e){
			throw new DAOException("Erro salvar Cliente", e);
		}
	...
	public void remover(TituloPerito tituloPerito) throws DAOException {
		logger.debug("Entrou ClienteDAOJPA");
		EntityManager manager = null;
		try{
			manager = PersistenceUtil.currentEntityManager();
			...
			manager.remove(tituloPeritoManaged);
			manager.flush();
		}catch(Exception e){
			throw new DAOException("Erro salvar Cliente", e);
		}
	...
	public ArrayList<AbstractEntity> pesquisar() throws DAOException {		
		logger.debug("Entrou TituloPeritoDAOJPA");
		EntityManager manager = null;
		try{
			manager = PersistenceUtil.currentEntityManager();
			StringBuffer queryStr = new StringBuffer();
			
	        queryStr.append("from TituloPerito p order by titulo");
