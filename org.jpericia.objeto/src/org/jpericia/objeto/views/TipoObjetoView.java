...
import org.jpericia.objeto.actions.TipoObjetoEditarAction;
import org.jpericia.objeto.actions.TipoObjetoRemoverAction;
...
import org.jpericia.objeto.views.sorters.TipoObjetoSorter;

public class TipoObjetoView extends ViewPart implements PropertyChangeListener
{
	public static final String VIEW_ID = "org.jpericia.objeto.views.tipoObjetoView";
	
	private TipoObjeto tipoObjeto;
	
	private TipoObjetoRemoverAction removerAction;
	
	private TipoObjetoEditarAction editarAction;
	...
	ViewerFilter filter = new ViewerFilter()
	{
		public boolean select(Viewer viewer1, Object parentElement,
				Object element)
		{
			return (TipoObjetoView.this.tipoText.getText().length() == 0 || ((TipoObjeto) element)
					.getTitulo()
					.matches(
							replaceWildCard(TipoObjetoView.this.tipoText.getText())));
		}
	};
	...
	class ViewContentProvider implements IStructuredContentProvider
	{
		...
		{
			return ((TipoObjetoListener) parent).toArray(new TipoObjeto[0]);
		}
	}
			...
			ITableLabelProvider
	{
		public String getColumnText(Object obj, int index)
		{
			...
			{
			case 0:
				returnValue = ((TipoObjeto) obj).getCodigo().toString();
				break;
			case 1:
				returnValue = ((TipoObjeto) obj).getTitulo();
				break;
			default:
				break;
			}
	...
	public void createPartControl(Composite parent)
	{
		...
		infoLabel.setText(Messages.infoTipoObjeto);
		...
		applyButton.addListener(SWT.Selection, new Listener()
		{
			public void handleEvent(Event event)
			{
				try
				{
					TipoObjetoView.this.viewer.setInput(TipoObjetoDelegate.getInstance().pesquisar());
				}
				...
				TipoObjetoView.this.viewer.addFilter(TipoObjetoView.this.filter);
			}
		});
		...
		clearButton.addListener(SWT.Selection, new Listener()
		{
			public void handleEvent(Event event)
			{
				TipoObjetoView.this.viewer.removeFilter(TipoObjetoView.this.filter);
			}
		});
		...
		Listener sortListener = new Listener()
		{
			public void handleEvent(Event e)
			{
				...
				TableColumn sortColumn = TipoObjetoView.this.viewer.getTable()
						.getSortColumn();
				TableColumn currentColumn = (TableColumn) e.widget;
				int dir = TipoObjetoView.this.viewer.getTable().getSortDirection();
				...
				{
					TipoObjetoView.this.viewer.getTable().setSortColumn(
							currentColumn);
					dir = SWT.UP;
				}
				...
				{
					sortIdentifier = TipoObjetoSorter.CODIGO;
				}
				if (currentColumn == tc1)
				{
					sortIdentifier = TipoObjetoSorter.TITULO;
				}
				TipoObjetoView.this.viewer.getTable().setSortDirection(dir);
				TipoObjetoView.this.viewer.setSorter(new TipoObjetoSorter(
						sortIdentifier, dir));
			}
		};
		...
		try
		{
			...
			TipoObjetoDelegate.getInstance().pesquisar().addPropertyChangeListener(
					this);
		}
		catch (BusinessDelegateException e)
		{
	...
	public void propertyChange(final PropertyChangeEvent evt)
	{
		getViewSite().getShell().getDisplay().asyncExec(new Runnable()
		{
			...
			public void run()
			{
				...
				{
					TipoObjetoView.this.viewer.refresh();
				}
	...
	public void dispose()
	{
		try
		{
			TipoObjetoDelegate.getInstance().pesquisar().removePropertyChangeListener(
					this);
		}
		catch (BusinessDelegateException e)
		{
	...
	private void createToolBarButtons()
	{
		...
		viewer.addSelectionChangedListener(new ISelectionChangedListener()
		{
			public void selectionChanged(SelectionChangedEvent event)
			{
				...
				setTipoObjeto((TipoObjeto)((StructuredSelection)event.getSelection()).getFirstElement());
			}
		});
	}
