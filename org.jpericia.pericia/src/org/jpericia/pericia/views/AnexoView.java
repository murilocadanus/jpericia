...
import org.jpericia.pericia.actions.AnexoEditarAction;
import org.jpericia.pericia.actions.AnexoRemoverAction;
...
import org.jpericia.pericia.views.sorters.AnexoSorter;

public class AnexoView extends ViewPart implements PropertyChangeListener
{
	public static final String ID = "org.jpericia.pericia.views.anexoView"; //$NON-NLS-1$
	
	private Anexo anexo;
	
	private AnexoRemoverAction removerAction;
	
	private AnexoEditarAction editarAction;
	...
	ViewerFilter filter = new ViewerFilter()
	{
		public boolean select(Viewer viewer1, Object parentElement,
				Object element)
		{
			return (AnexoView.this.tituloText.getText().length() == 0 || ((Anexo) element)
					.getTitulo()
					.matches(
							replaceWildCard(AnexoView.this.tituloText.getText())));
		}
	};
	...
	class ViewContentProvider implements IStructuredContentProvider
	{
		...
		{
			return ((AnexoListener) parent).toArray(new Anexo[0]);
		}
	}
			...
			ITableLabelProvider
	{
		public String getColumnText(Object obj, int index)
		{
			...
			{
			case 0:
				returnValue = ((Anexo) obj).getPericia().getTitulo();
				break;
			case 1:
				returnValue = ((Anexo) obj).getTitulo();
				break;				
			default:
				break;
			}
	...
	public void createPartControl(Composite parent)
	{
		...
		infoLabel.setText(Messages.infoAnexo);

		//Campo pericia
		Label userLabel = new Label(parent, SWT.NONE);
		userLabel.setText(Messages.pericia);
		gd = new GridData(SWT.BEGINNING, SWT.CENTER, false, false);
		gd.widthHint = 100;

		userLabel.setLayoutData(gd);
		this.periciaCmo = new ComboExtended(parent, SWT.BORDER);
		gd = new GridData(SWT.BEGINNING, SWT.CENTER, false, false);
		gd.widthHint = 200;
		this.periciaCmo.setLayoutData(gd);		
		...
		applyButton.addListener(SWT.Selection, new Listener()
		{
			public void handleEvent(Event event)
			{
				try
				{
					...
					//{
						AnexoView.this.viewer.setInput(AnexoDelegate.getInstance().pesquisar());
					//}
				}
				...
				AnexoView.this.viewer.addFilter(AnexoView.this.filter);
		...
		clearButton.addListener(SWT.Selection, new Listener()
		{
			public void handleEvent(Event event)
			{
				AnexoView.this.viewer.removeFilter(AnexoView.this.filter);
			}
		});
		...
		tc0.setText(Messages.pericia);
		tc0.setData(new ColumnWeightData(25));
		...
		Listener sortListener = new Listener()
		{
			public void handleEvent(Event e)
			{
				...
				TableColumn sortColumn = AnexoView.this.viewer.getTable()
						.getSortColumn();
				TableColumn currentColumn = (TableColumn) e.widget;
				int dir = AnexoView.this.viewer.getTable().getSortDirection();
				...
				{
					AnexoView.this.viewer.getTable().setSortColumn(
							currentColumn);
					dir = SWT.UP;
				}
...
/*				if (currentColumn == tc0)
				...
				AnexoView.this.viewer.getTable().setSortDirection(dir);
				AnexoView.this.viewer.setSorter(new AnexoSorter(
						sortIdentifier, dir));
			}
		};
	...
	public void dispose()
	{
		try
		{
			AnexoDelegate.getInstance().pesquisar().removePropertyChangeListener(
					this);
		}
		catch (BusinessDelegateException e)
		{
	...
	private void createToolBarButtons()
	{
		...
		viewer.addSelectionChangedListener(new ISelectionChangedListener()
		{
			public void selectionChanged(SelectionChangedEvent event)
			{
				...
				setAnexo((Anexo)((StructuredSelection)event.getSelection()).getFirstElement());
			}
		});
	}
